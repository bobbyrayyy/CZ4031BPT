...
void BPTree::insert(keys_struct x)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // Make new leaf node point to the next leaf node - so we can traverse at the lowest leaf level for ranged queries
            newLeaf->ptr[newLeaf->size] = cursor->ptr[MAX];
            cursor->ptr[MAX] = NULL;    // The last pointer in the rightmost leaf node points to NULL
